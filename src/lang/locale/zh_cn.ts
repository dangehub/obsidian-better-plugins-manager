export default {
    ...
    设置_基础设置_同步模式_标题: '同步模式',
    设置_基础设置_同步模式_描述: '仅导出：BPM 写入插件信息，不监听文件。双向：笔记中修改的元数据可写回 BPM。',
    设置_基础设置_同步模式_仅导出: '仅导出',
    设置_基础设置_同步模式_双向: '双向（监听并写回）',
    设置_基础设置_允许笔记启用_标题: '允许从笔记更改启用状态',
    设置_基础设置_允许笔记启用_描述: '在双向模式下开启后，修改笔记中的 bpm_rw_enabled 可以启用/禁用插件。默认关闭。BPM 自身不能被笔记禁用。',
    ...
    // Plugin Notes Export
    设置_基础设置_导出目录_验证失败: '导出目录无效：',
    通用_设置_保存成功: '设置已保存。',
    插件笔记_导出成功: '插件笔记已导出：{name}',
    插件笔记_导出失败: '插件笔记导出失败：{name}',
    插件笔记_全量导出中: '正在全量导出插件笔记…',
    插件笔记_全量导出完成: '已导出 {written}/{total} 个插件笔记（跳过 {skipped}，错误 {errors}）。',
    插件笔记_冲突跳过: '插件 {id} 的笔记因重复 ID 已跳过。',
}
